...
export const getResultByRollNoFromSite = async (
  ...
  if (!isValidRollNumber(rollNo)) {
    return res
      .status(400)
      .json({ message: "Invalid roll number", error: true, data: null });
  }
  try {
    ...
    return res
      .status(500)
      .json(safeErrorBody("Failed to fetch result", (err as Error)?.message));
  }
};
...
export const getResult = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ data: resultData, message: "Result found", error: false });
  } catch (err) {
    console.error("getResult error:", err);
    return res
      .status(500)
      .json(safeErrorBody("An error occurred", (err as Error)?.message));
  }
};
...
export const updateResult = async (req: Request, res: Response) => {
  ...
  // Everything else is re-scraped below; rollNo, rank and timestamps must never come from the body.
  const custom_attributes = rawResultSchema
    .pick({ gender: true })
    .partial()
    .safeParse(req.body ?? {});
  ...
  try {
    ...
    const updated = await ResultModel.findByIdAndUpdate(
      resultData._id,
      {
        $set: {
          ...
          programme: result.programme,
          semesters: result.semesters,
          // findByIdAndUpdate skips the pre-save hook that normally keeps this in sync.
          latestCgpi: result.semesters[result.semesters.length - 1]?.cgpi ?? 0,
        },
      },
    ...
    return res
      .status(500)
      .json(safeErrorBody("An error occurred", (error as Error)?.message));
  }
};
...
// One request scrapes sequentially, so cap it well below what would outlive an HTTP timeout.
const MAX_BULK_UPDATE = 200;
const MAX_BULK_DELETE = 5000;

function readRollNos(
  body: unknown,
  max: number
): { rollNos: string[] } | { error: string } {
  const parsed = z
    .object({ rollNos: z.array(z.string()).max(max) })
    .safeParse(body);
  if (!parsed.success) {
    return { error: `rollNos must be an array of at most ${max} roll numbers` };
  }
  const rollNos = parsed.data.rollNos.filter(isValidRollNumber);
  return rollNos.length > 0
    ? { rollNos }
...
export const bulkUpdateResults = async (req: Request, res: Response) => {
  try {
    const input = readRollNos(req.body, MAX_BULK_UPDATE);
    if ("error" in input) {
      return res
        .status(400)
        .json({ error: true, message: input.error, data: null });
    }
    const validatedRollNos = input.rollNos;
    const BATCH_SIZE = 8;
    const result = {
      total: validatedRollNos.length,
      updated: 0,
      errors: [] as { rollNo: string; error: string }[],
    };
    ...
    for (let i = 0; i < validatedRollNos.length; i += BATCH_SIZE) {
      ...
      const outcomes = await Promise.allSettled(
        batch.map((rn) => scrapeAndSaveResult(rn))
      );

      outcomes.forEach((outcome, idx) => {
        const rn = batch[idx];
        // scrapeAndSaveResult reports failures as `success: false` rather than rejecting.
        if (outcome.status === "fulfilled" && outcome.value.success) {
          result.updated += 1;
        } else if (outcome.status === "fulfilled") {
          result.errors.push({
            rollNo: rn,
            error: outcome.value.error || "Scraping failed",
          });
        } else {
          result.errors.push({
            rollNo: rn,
            error: outcome.reason ? String(outcome.reason) : "Unknown error",
          });
        }
      });
    }
    ...
    return res
      .status(200)
      .json({ error: false, message: "Bulk update successful", data: result });
  } catch (error) {
...
export const bulkDeleteResults = async (req: Request, res: Response) => {
  try {
    const input = readRollNos(req.body, MAX_BULK_DELETE);
    if ("error" in input) {
      return res
        .status(400)
        .json({ error: true, message: input.error, data: null });
...
export const assignRankToResults = async (req: Request, res: Response) => {
  try {
    ...
    try {
      const resultsWithRanks = await ResultModel.aggregate(
        pipelines["assign-rank"]
      ).allowDiskUse(true);

      const bulkUpdates = resultsWithRanks.map((r) => ({
        updateOne: { filter: { _id: r._id }, update: { rank: r.rank } },
      }));
      ...
      for (let i = 0; i < bulkUpdates.length; i += CHUNK_SIZE) {
        ...
        console.log(
          `Processed chunk ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(bulkUpdates.length / CHUNK_SIZE)}`
        );
      }
...
export const assignBranchChangeToResults = async (
  ...
  try {
    ...
    const results = await ResultModel.aggregate(
      pipelines["assign-branch-change"]
    ).allowDiskUse(true);

    const bulkOperations = results.map((result) => {
      // Efficient frequency count
      const courseCount = (result.uniquePrefixes || []).reduce(
        (acc: Record<string, number>, prefix: string) => {
      ...
      const maxPrefix =
        Object.keys(courseCount).find(
          (prefix) => courseCount[prefix] === maxCourses
        ) || null;
      const department = getDepartmentCoursePrefix(maxPrefix || "");

      // The pipeline doesn't project gender, so only the branch is ever rewritten here.
      const updates: { branch: string } = { branch: result.branch };
...
export const importFreshers = async (req: Request, res: Response) => {
  try {
    ...
    const inserted = await ResultModel.insertMany(results, {
      ordered: false,
    }).catch((e) => {
    ...
    return res.status(200).json({
      ...
      data: {
        timeTaken: `${(Date.now() - time) / 1000}s`,
        lastUpdated: new Date().toISOString(),
        results: `${inserted.length} freshers imported`,
      },
    });
  } catch (error) {
...
export const getResultsByBatch = async (req: Request, res: Response) => {
  try {
    ...
    if (!Number.isFinite(batch) || batch <= 0) {
      return res
        .status(400)
        .json({ message: "Invalid batch", error: true, data: null });
    }

    const sortBy =
      (req.query.sortBy as string) === "latestCgpi"
        ? "latestCgpi"
        : "rank.batch";
    ...
    const rows = docs.map((d: any) =>
      [d.rollNo, d.name, d.latestCgpi ?? 0, d.rank?.batch ?? 0]
        .map(escape)
        .join(",")
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="results-batch-${batch}.csv"`
    );
