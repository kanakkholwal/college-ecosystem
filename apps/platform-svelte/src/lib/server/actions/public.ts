...
import { sessions, users } from '$lib/server/db/schema/auth-schema';
import { appConfig } from '$lib/config/project';
import { extractVisitorCount, getRepoStats, type PublicStatsType } from '$lib/server/third-party/github';
...
export async function getPublicStats(): Promise<PublicStatsType> {
	...
	const sessionCount =
		session_result.status === 'fulfilled' ? (session_result.value[0]?.count ?? 0) : 0;
	const userCount = user_result.status === 'fulfilled' ? (user_result.value[0]?.count ?? 0) : 0;
	const githubStats =
