...
export async function reactToPost(
  postId: string,
  reaction: (typeof postReactions)[number]
) {
  const headerList = await headers();
  const ip =
    headerList.get("x-forwarded-for") ||
    headerList.get("x-real-ip") ||
    headerList.get("remoteAddress") ||
    "unknown";
  if (!rateLimit(ip)) {
...
export async function createWhisperPost(rawPost: unknown): Promise<boolean> {
  ...
  if (!validation.success) {
    throw new Error(
      "Invalid post data: " + JSON.stringify(validation.error.issues)
    );
  }
  const session = await getSession();
  if (!session) {
...
export async function getWhisperPostById(
  postId: string
): Promise<WhisperPostT> {
  if (!postId) {
...
export async function updateWhisperPoll(
  postId: string,
  updatedPoll: PollT
): Promise<PollT> {
...
export async function editWhisperPost(
  postId: string,
  rawPost: unknown
): Promise<WhisperPostT> {
  if (!postId) {
  ...
  try {
    ...
    const { content_json, visibility, category, poll, pseudo } =
      validation.data;
...
export async function patchWhisperPost(
  postId: string,
  patch: patchMethod,
  payload: Partial<WhisperPostT>
): Promise<WhisperPostT> {
  if (!postId) {
    throw new Error("Invalid postId");
  }
