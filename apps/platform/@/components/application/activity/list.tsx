    ...
    >
      <div className="text-left pt-4 pl-4">
        <h4 className="text-base text-foreground font-semibold">
          {changeCase(targetModel, "camel_to_title")} Activity
        </h4>
        <p className="text-sm text-muted-foreground">
          People who interacted with this{" "}
          {changeCase(targetModel, "camel_to_title")}.
        </p>
      </div>
      <Tabs defaultValue={keys[0]} className="w-full bg-card rounded-lg">
            ...
            {keys.map((key) => {
              ...
              return (
                <TabsTrigger
                  value={key}
                  className="gap-2"
                  key={`activity-${key}-tab`}
                >
                  <Icon
                    name={value.icon}
                    className={cn("size-4", value.iconColor)}
                  />
                  {changeCase(key, "camel_to_title")}{" "}
        ...
        <div className="p-4">
          {keys.map((key) => {
            const value = activity[key];
            return (
              <TabsContent value={key} key={`activity-${key}-content`}>
    ...
    >
      <div className="relative">
        ...
        <span className="absolute bottom-0 right-0 translate-2 z-5 size-4">
          <Icon name={icon} className={cn("size-3", iconColor)} />
        </span>
      </div>
