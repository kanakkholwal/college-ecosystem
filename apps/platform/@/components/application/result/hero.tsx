...
export function HeroLabel({ session }: { session: string }) {
  return (
    <div className="mb-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <Badge
        variant="outline"
        className="rounded-full px-3 py-1 border-primary/20 bg-primary/5 text-primary text-xs font-medium backdrop-blur-sm gap-1.5 hover:bg-primary/10 transition-colors cursor-pointer"
      >
...
export function HeroQuickFilters({
  ...
  filters: { batch: string; programme: string }[];
}) {
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 mt-5 text-sm text-muted-foreground">
      <span className="text-xs font-medium uppercase tracking-wide opacity-70 mr-1">
        Quick Filters:
      </span>
      {filters.map((filter) => (
        <Link
...
export function HeroStats({
  ...
  return (
    <div className="mt-10 grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-16 border-t border-border/40 pt-8 w-full max-w-3xl">
