  ...
  return (
    <div className="flex flex-col items-center space-y-8 w-full max-w-3xl mx-auto">
      ...
      <div className="w-full overflow-x-auto pb-8 pt-4 px-2 flex justify-center bg-muted/30 rounded-2xl border border-dashed backdrop-blur-lg">
        ...
        >
                ...
                <div className="h-10 border-b-2 border-slate-200 relative">
                  <span
                    className={cn(
                      ...
                      "absolute bottom-1 left-0 text-xl text-slate-700"
                    )}
                  >
                ...
                <div className="h-10 border-b-2 border-slate-200 relative">
                  {isSigned ? (
                    <div className="absolute bottom-0 left-0 flex items-end">
                      ...
                      <ShieldCheck
                        className={cn("h-4 w-4 mb-2 ml-1", statusTheme.text)}
                      />
                    </div>
                  ) : (
                    <span className="absolute bottom-1 text-xs text-slate-400 italic">
                      Pending Approval
                    </span>
                  )}
                </div>
          ...
          <div className="relative h-px w-full border-t-2 border-dashed border-slate-200/60 z-10">
            ...
            <div
              className={cn(
                "absolute -left-3 top-1/2 -translate-y-1/2 h-6 w-6 rounded-full box-content border-[10px] border-transparent",
                statusTheme.bg
              )}
            ...
            <div
              className={cn(
                "absolute -right-3 top-1/2 -translate-y-1/2 h-6 w-6 rounded-full box-content border-[10px] border-transparent",
                statusTheme.bg
              )}
              style={{ boxShadow: `inset 5px 0 5px -5px rgba(0,0,0,0.1)` }}
            ></div>
          ...
          <div
            className={cn(
              "p-6 flex flex-col items-center text-center relative transition-colors duration-300",
              statusTheme.bg
            )}
          >
            {/* Status Badge */}
            <div
              className={cn(
                "w-full py-3 rounded-xl border flex flex-col items-center gap-1 mb-6 shadow-sm bg-white/80 backdrop-blur-sm",
                statusTheme.border
              )}
            >
            ...
            {outpass.status !== "rejected" && outpass.status !== "pending" ? (
              <>
                ...
                <div className="space-y-1 mb-6">
                  ...
                  <p className="font-mono text-xs text-slate-400 tracking-widest">
                    {outpass._id.toString().slice(-12)}
                  </p>
                </div>
              </>
            ) : (
            ...
            <div className="w-full border-t border-slate-900/10 pt-4 grid grid-cols-2 gap-4">
              <div className="text-left">
                <Label className={cn("text-slate-500", statusTheme.text)}>
                  HOSTEL
                </Label>
                <div className="text-slate-900 font-bold text-sm uppercase truncate">
                  {outpass.hostel.name}
                </div>
              </div>
              <div className="text-right">
      ...
      <p className="text-center text-xs text-muted-foreground max-w-md leading-relaxed">
        This digital pass is official property of {orgConfig.name}. Misuse is
        punishable. The barcode contains encrypted biometric verification data.
      </p>
    </div>
  );
}
