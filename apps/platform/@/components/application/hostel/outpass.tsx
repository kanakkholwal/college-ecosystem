...
export function OutpassDetails({
  ...
  return (
    ...
    >
      ...
      >
        <div className={cn("h-full w-1/4", theme.accent)}></div>{" "}
        {/* Small color accent dot/line on the left */}
      </div>
      ...
      <div className="p-5 pt-6 flex justify-between items-center gap-3">
        <div className="flex gap-3 items-center overflow-hidden">
          <Avatar className="h-11 w-11 border border-border/50 shrink-0">
            <AvatarFallback className="bg-primary/5 text-primary font-bold text-base">
              {outpass.student?.name?.slice(0, 2).toUpperCase() || (
        ...
        <Badge
          className={cn(
            "uppercase text-[10px] font-bold tracking-wider gap-1.5 pl-2 pr-3 h-6 shadow-none border-0 transition-colors",
            theme.badge
          )}
        >
      ...
      <div className="p-5 space-y-5">
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-[11px] uppercase font-semibold text-muted-foreground mb-1">
              Departure
            </span>
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col text-sm">
                <span className="font-bold leading-none">
                  {format(new Date(outpass.expectedOutTime || ""), "dd MMM")}
                </span>
                <span className="text-xs font-medium text-muted-foreground/90">
                  {format(new Date(outpass.expectedOutTime || ""), "HH:mm")}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col text-right">
            ...
            <div className="flex items-center justify-end gap-2">
              <div className="flex flex-col text-sm">
                <span className="font-bold leading-none">
                  {format(new Date(outpass.expectedInTime || ""), "dd MMM")}
                </span>
                <span className="text-xs font-medium text-muted-foreground/90">
                  {format(new Date(outpass.expectedInTime || ""), "HH:mm")}
                </span>
              </div>
              <CalendarClock className={cn("h-4 w-4", theme.accent)} />
            </div>
          ...
          {outpass.status === "pending" ? (
            ...
            <div className="text-xs text-muted-foreground italic w-full text-center py-1">
              {outpass.status === "approved"
                ? "Action completed. Outpass is valid."
