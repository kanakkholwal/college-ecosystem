...
import { BarChart2, MoreHorizontal, Pin, PinOff, Trash2 } from "lucide-react";
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                ...
                onClick={() =>
                  toast.promise(deleteWhisperPost(post._id!), {
                    loading: "Deleting...",
...
export function WhisperCardFooter({
  post,
  user,
  className,
}: {
  ...
  const [optimisticPost, setOptimisticPost] = useOptimistic(
    post,
    (current, action: { type: ReactionType }) => {
      if (!user) return current;
      const exists = current.reactions.find(
        (r) => r.type === action.type && r.userId === user.id
      );
      ...
      const newReactions = exists
        ...
        : [
            ...current.reactions.filter((r) => r.userId !== user.id),
            { type: action.type, userId: user.id },
          ]; // Add/Replace
    ...
    <div className={cn("flex w-full justify-between items-center", className)}>
      <div className="flex gap-1">
        {REACTION_OPTIONS.map((r) => {
          const count = optimisticPost.reactions.filter(
            (rx) => rx.type === r.value
          ).length;
          const isActive = optimisticPost.reactions.some(
    ...
    if (node.text) {
      ...
      if (node.marks) {
        node.marks.forEach((mark) => {
          ...
          if (mark.type === "code")
            text = `<code class="bg-muted px-1 rounded text-xs font-mono">${text}</code>`;
        });
      }
      html += text;
    }
