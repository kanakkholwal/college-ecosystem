...
import {
  placementStorySchema,
  type PlacementStoryInput,
} from "~/constants/placement-story";
    ...
    <div className="max-w-3xl mx-auto py-12 px-4">
      <div className="mb-10 text-center md:text-left">
        ...
        <p className="text-muted-foreground text-lg">
          Help juniors navigate their career path by documenting your interview
          experience.
        </p>
      </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card className="border-l-4 border-l-primary shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl">
              <PiBuildingsDuotone className="text-primary h-6 w-6" /> Offer
              Details
            </CardTitle>
          </CardHeader>
          <CardContent className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(val: any) => form.setValue("offerType", val)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select type" />
                </SelectTrigger>
                <SelectContent>
                  {["Internship", "FTE", "PPO", "Intern+FTE", "Other"].map(
                    (type) => (
              ...
              {form.formState.errors.offerType && (
                <p className="text-xs text-red-500">
                  {form.formState.errors.offerType.message}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                placeholder="e.g. 1.5L/mo or 20 LPA"
                {...form.register("ctc")}
              />
            </div>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl">
              <PiBriefcaseDuotone className="text-primary h-6 w-6" /> The
              Process
            </CardTitle>
            <CardDescription>
              Be detailed about rounds, questions asked, and the difficulty
              level.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              />
              {form.formState.errors.selectionProcess && (
                <p className="text-xs text-red-500">
                  {form.formState.errors.selectionProcess.message}
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              {form.formState.errors.preparationStrategy && (
                <p className="text-xs text-red-500">
                  {form.formState.errors.preparationStrategy.message}
                </p>
              )}
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6 space-y-6">
            <div className="space-y-4">
              ...
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className="flex gap-2 items-start animate-in slide-in-from-left-2 duration-300"
                >
                  <div className="grid gap-2 flex-1 md:grid-cols-2">
                    <Input
                      placeholder="Resource Title (e.g. Blind 75)"
        ...
        <div className="pt-4">
          ...
          >
            Submit Experience{" "}
            <PiPaperPlaneRightDuotone className="ml-2 h-5 w-5" />
          </Button>
        </div>
      </form>
    </div>
