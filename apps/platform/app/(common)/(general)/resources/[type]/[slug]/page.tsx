              ...
              <div className="flex flex-wrap justify-center gap-2 mb-6">
                {frontmatter.tags.map((tag: string) => (
                  <Badge
                    key={tag}
                    variant="secondary"
                    className="px-3 py-1 font-mono text-xs uppercase tracking-wider text-muted-foreground bg-muted/50 border-border/50"
                  >
                    {tag}
                  </Badge>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="relative rounded-2xl overflow-hidden border border-border/50 shadow-sm aspect-video bg-muted">
                  <Image
                    src={
                      ...
                      `/og/resources/${resolvedParams.type}/${resolvedParams.slug}`
                    }
            ...
            <aside className="lg:col-span-4 space-y-8">
              <div className="sticky top-36 space-y-8">
                ...
                {response.data.toc && response.data.toc.length > 0 && (
                  <div className="rounded-xl border border-border/50 bg-card/50 p-6 backdrop-blur-sm">
                    <h3 className="flex items-center gap-2 font-semibold text-foreground mb-4">
                      <FileText className="size-4 text-primary" /> Table of
                      Contents
                    </h3>
                ...
                {/* Alternate Reads */}
                {frontmatter.alternate_reads &&
                  frontmatter.alternate_reads.length > 0 && (
