    ...
    <Card className="overflow-hidden border border-border/60 shadow-sm transition-all hover:shadow-md hover:border-border/80 group">
      ...
      <div className="bg-muted/30 border-b border-border/50 px-5 py-3 flex flex-wrap justify-between items-center gap-3">
        ...
        <Badge
          variant="outline"
          className={cn(
            "pl-1.5 pr-2.5 py-0.5 gap-1.5 border shadow-none font-medium transition-colors",
            statusConfig.badge
          )}
        >
            ...
            <div className="flex gap-4 items-start">
              ...
              >
                <Home
                  className={cn(
                    "h-3.5 w-3.5",
                    isLateReturn ? "text-red-500" : "text-muted-foreground"
                  )}
                />
              </div>
              <div className="space-y-1">
...
function getDuration(start: Date, end: Date) {
  ...
  if (diffHrs > 24)
    return `${Math.floor(diffHrs / 24)} Days ${diffHrs % 24} Hrs`;
  if (diffHrs > 0)
    return `${diffHrs} Hr ${diffMins > 0 ? `${diffMins} Min` : ""}`;
