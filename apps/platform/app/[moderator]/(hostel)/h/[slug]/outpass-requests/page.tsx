...
import { AlertCircle, FileText, LayoutGrid, RefreshCcw } from "lucide-react";
...
export default function OutPassRequestsPage() {
  ...
  const { data, error, isLoading, mutate, isValidating } =
    useSWR<OutpassResponse>("/api/outpass/list?slug=" + slug, fetcher, {
  ...
  const getStatusCount = (status: string) => {
    return (
      data?.groupedOutPasses?.[status as OutPassType["status"]]?.length || 0
    );
  };
      ...
      <Tabs defaultValue={OUTPASS_STATUS[0]} className="w-full">
        ...
        <div className="mt-6 min-h-[400px]">
          ...
          <ConditionalRender condition={!!data}>
            {OUTPASS_STATUS.map((status) => {
              const items =
                data?.groupedOutPasses[status as OutPassType["status"]] || [];
