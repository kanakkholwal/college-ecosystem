...
export default function ScrapeResultPage() {
  ...
  const resetTimeout = useCallback(() => {
    ...
    taskData.status,
    taskData.list_type,
  ]); // Dependencies needed for resume logic
  ...
  const handleStartScraping = useCallback(
    (payload?: {
      ...
      if (payload) {
        ...
        if (payload.task_resume_id)
          url.searchParams.set("task_resume_id", payload.task_resume_id);
      } else {
      ...
      const onTaskStatus = (e: MessageEvent) => {
        ...
        try {
          ...
          // Completion Check
          if (
            payloadData.status === TASK_STATUS.COMPLETED ||
            payloadData.status === TASK_STATUS.CANCELLED
          ) {
            if (payloadData.status === TASK_STATUS.COMPLETED)
              toast.success("Scraping completed.");
            if (payloadData.status === TASK_STATUS.CANCELLED)
              toast("Scraping cancelled.");
    ...
    [listType, router, resetTimeout, closeConnection]
  ); // Added dependencies
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage real-time scraping tasks and view historical logs.
          </p>
        </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>
                Recent scraping sessions and their outcomes.
              </CardDescription>
            </div>
            <Button
              variant="ghost"
...
function LiveMonitorCard({ task, streaming, error, onStop }: any) {
  const percent =
    task.processable > 0
      ? Math.min(100, Math.round((task.processed / task.processable) * 100))
      : 0;
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              {streaming && (
                <Badge
                  variant="secondary"
                  className="animate-pulse text-blue-600 bg-blue-50"
                >
                  Live
                </Badge>
              )}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-medium text-muted-foreground">
                <span>Progress</span>
                <span>
                  {percent}% ({task.processed} / {task.processable})
                </span>
              </div>
        ...
        {task.failed > 0 ? (
          <ResponsiveDialog
            btnProps={{
              variant: "ghost",
              size: "sm",
              className: "text-red-600 h-6 px-2 text-xs hover:bg-red-50",
              children: <span>{task.failed} (View)</span>,
            }}
            title="Failed Items"
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {task.processed < task.processable && (
              <DropdownMenuItem
                onClick={() =>
                  onAction(task._id, EVENTS.TASK_PAUSED_RESUME, task.list_type)
                }
              >
            ...
            {isComplete && task.failed > 0 && (
              <DropdownMenuItem
                onClick={() =>
                  onAction(task._id, EVENTS.TASK_RETRY_FAILED, task.list_type)
                }
              >
...
function FailedRollNumbers({ task }: { task: taskDataType }) {
  if (!task.failedRollNos || task.failedRollNos.length === 0)
    return (
      <p className="text-sm text-muted-foreground">No failed items recorded.</p>
    );
